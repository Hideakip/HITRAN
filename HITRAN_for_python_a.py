...
class HITRAN():
    ...
    def __init__(self):
        ...
        Tl=298.15                  #Reference temperature k
    ...
    def do_gases(self):
        ...
        for gasn in gas:
            ...
            if mode==0:     #stick spectra
                 ...
                 #Feed chucnked list of wavenumbers here##################################
                 #########################################################################
                 ...
                 t0=time.time()
                 alpha=self.scan_peak_ranges(wnpeakS,wni,St,Nx,gammap,gammad,max_alpha,imax)   #Returns nothing because data written to hdf5 file
    ...
    def scan_peak_ranges(self,wnpeakS,wni,St,Nx,gammap,gammad,max_alpha,imax):

        i=0 #Index counter for position in list of peaks wmpeakS
        N=1e-5 #Low intensity limit of calculation. Fraction of highest peak in range
        peakmin=max_alpha*N
        ...
        for peak in wnpeakS:
            ...
            b=peak+sigma
            peakrange=wni[[wni>=a]]
            peakrange=peakrange[peakrange<=b]
            ...
            f=wni[wni<a]
            b=wni[wni>b]
            front=size(f)  #wni[wni<a])
            back=size(b) #wni[wni>b])
            #print peakrange[0],a
            #print peakrange[-1],b[0],b[-1],wni[-1]
            fronts=zeros(front)
            backs=zeros(back)
            gv=self.Voigt1(dvp,gammap[i],gammad[i])
            alpha=self.alpha(gv,St[i],Nx)
            alpha=hstack((fronts,alpha,backs))
            alphatot+=alpha
            i+=1
...
#        print "35456werweewewew"
    ...
    def Voigt1(self,dvp,gammap,gammad):
    ...
    def alpha(self,gv,St,Nx):

        alpha=St*gv*Nx*1.0e-6      #!!!!!!!!!!!!Dropped a factor of 1e-6 wrt MS reference data for H20
        return alpha
    ...
    def write_file(self,wnout,dw,alphaout):
        ...
        foutpath="C:\Users\Hideaki\Dropbox\Scytronix\Software\Hitran\HITRAN v7\Out\\"
    ...
    def plot_spectra(self,gas,wnout,alphaout):
        gas,concentration,isotope =self.read_gas_species()
        Ngases=len(gas)
        plottype=1
        ...
        for i in range (0,Ngases): 
            if plottype==0:
                plt.plot(wnout[i],alphaout[i], label=gas[i])
                plt.legend(loc='upper right')
            if plottype==1:
                lamb=1e4/array(wnout[i])
                plot(lamb,(alphaout[i]),label=gas[i])
                plt.legend(loc='upper right')
            
        show()

    def transmission(self,wni,sourcex,sourcey,alpha,L):
        f=interpolate.interp1d(sourcex,sourcey)
        Pnew=f(wni)
        sig=Pnew*exp(-alpha*L)
